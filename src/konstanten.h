#ifndef KONSTANTEN_H
#define KONSTANTEN_H

static const float SPIELEBENE = -4500;
static const float SPIELERBESCHLEUNIGUNG = 0.1;
static const float SPEEDSTRENGTH = 7;
static const float PLAYERHEAVINESS = 10;
static const float SPEEDBOOST = 5;

static const float MUNITIONSHEAVINESS = 10;
static const float MUNITIONSBESCHLEUNIGUNG = 0.9;
static const int WAPPONREALOADTIME = 20;
static const float PROJECTIELESPEED = 13;

static const float PLANETHEAVINESS = 2400000000000;
static const float SUNHEAVINESS =   20000000000000;
static const float PI = 3.14159265358979323846264338327950288419716939937510;


static const int SPIELFELDBREITE = 35000;
#endif